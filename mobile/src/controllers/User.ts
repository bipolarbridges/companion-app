...
import {
    ...
    logger,
} from 'common/controllers/UserController';
...
import {
    ...
    IAssessmentsController,
} from 'src/controllers/Assessments';
...
export class UserController
    ...
    constructor(auth: IAuthController) {
        ...
        this.disposer.add(
            auth.onPreProcessUser.on((u) => this.onPreProcessAuthUser(u)),
            'onPreProcessAuthUser',
        );
        ...
        this.disposer.add(
            autorun(() => {
                ...
                const acccountId =
                    (this._activeAccount && this._activeAccount.id) || null;
                const coachId =
                    (this._activeAccount && this._activeAccount.coachId) ||
                    null;
    ...
    get accountMissing() {
        return super.accountMissing || this.clientAccountMissing;
    }

    get clientAccountMissing() {
        return this._activeAccount === false;
    }

    get prompts() {
        return this._prompts;
    }
    get targetRole(): UserRoles {
    ...
    get recordsLastWeek() {
        return this._recordsLastWeek.value;
    }
    ...
    get documents() {
        if (!this._documents) {
            ...
            if (this._activeAccount) {
                this._documents?.setAccount(
                    this._activeAccount.coachId,
    ...
    @computed
    get displayName() {
        return NamesHelper.ensureFromUsers(this.user, this.auth.authUser)
            .displayName;
    }
    ...
    get onboardingDayIndex(): number | null {
        return (
            this.user?.client &&
            this.activeAccount &&
            getDayIndex(this.user.client.journalsHistory)
        );
    }
        ...
        if (authUser) {
            ...
            if (savedUserId !== authUser.uid) {
                ...
                const result = await Firebase.Instance.getFunction(
                    UsersFunctions.TryCreateClientUser,
                ).execute();
    ...
    protected async postProcessUser(user: FullUser, isUpdating?: boolean) {
        ...
        if (UserRoles.Helper.contains(user.roles, UserRoles.Client)) {
            ...
            const clientUnsub = await RepoFactory.Instance.clients.getClient(
                user.id,
                (c) => this.processClient(userCopy, c),
            );
            this.disposer.add(clientUnsub, 'GETCLIENT');

            const accountsUnsub = await RepoFactory.Instance.clients.getAccounts(
                user.id,
                {},
                (accounts) => this.processAccounts(accounts, user.id),
            );
    ...
    protected async onUserChanged(user: FullUser, isUpdating?: boolean) {
        if (!isUpdating && user) {
            this._records.weakValue?.setClient(
                this.activeAccount.coachId,
                this.user.id,
                this.user.displayName,
            );
            this._recordsLastWeek.weakValue?.setLoggerName(
                `${this.user.displayName || '??'}:week`,
            );
            ...
            if (process.appFeatures.MOBILE_STATIC_TIPS_ENABLED) {
                this.disposer.add(
                    await RepoFactory.Instance.staticTips.get((tips) => {
                        this._staticTips = tips?.filter((t) => !t.disabled);
                        logger.log(
                            `Loaded ${this._staticTips?.length} of ${tips?.length} STATIC TIPS`,
                        );
                    }),
                    'staticTips',
                );
    ...
    private processClient = (u: FullUser, client: Identify<ClientProfile>) => {
        ...
        const action = user.client ? 'Updating' : 'Initializing';
    ...
    private async processAccounts(
        accounts: ClientAccountIded[],
        userUid: string,
    ) {
        transaction(() => {
            this._accounts = accounts || [];
            this._activeAccount =
                this._accounts.find((s) => s.status === ClientStatus.Active) ||
                false;
        });
    ...
    public acceptConsent =
        process.appFeatures.MOBILE_SHOW_CONSENT &&
        (async (option: string) => {
            try {
                if (!this.user) {
                ...
                if (!option) {
                    throw new Error(
                        'acceptConsent: an option should be selected',
                    );
                }

                await Firebase.Instance.getFunction(
                    UsersFunctions.AcceptConsent,
                ).execute({ option });
                ...
                if (!this.user.client.consentAccepted) {
                    ...
                    this.user.client.consentAccepted = {
                        option,
                        date: Date.now(),
                    };
                }
