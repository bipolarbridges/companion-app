...
export default class ChooseDomainViewModel {
    ...
    get selectedDomains(): DomainIded[] {
    ...
    public getDomainDisplay(): string[] {
        ...
        const leftName =
            this._leftDomain > -1
                ? this._availableDomains[this._leftDomain].name
                : '';
        const rightName =
            this._rightDomain < this._availableDomains.length
                ? this._availableDomains[this._rightDomain].name
                : '';
        const mainName = this._availableDomains[this._mainDomain].name;
        const mainImportance = this._availableDomains[this._mainDomain]
            .importance;
    ...
    public selectDomain(domain: DomainIded): Boolean {
        ...
        AppViewModel.Instance.ChooseStrategy.setSelectedDomains(
            this._selectedDomains,
        );
        return true;
    }
