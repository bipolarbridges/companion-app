...
export default class ChooseStrategyViewModel {
    ...
    public setAvailableStrategies(strats: StrategyIded[]) {
        if (strats) {
            ...
            this.availableStrategies = this.availableStrategies.filter((s) =>
                s.associatedDomainNames.some((r) =>
                    this._selectedDomains.includes(r),
                ),
            );
    ...
    public filterAvailableStrategies(strategyDomain: string) {
        if (strategyDomain == null) {
            ...
            this.availableStrategies = this._allStrategies.filter((s) =>
                s.associatedDomainNames.includes(strategyDomain),
            );
        }
    }
