import { observer } from 'mobx-react';
 ...
 import { KeyboardAvoidingView, Platform, StyleSheet, Text, View, TextInput, Clipboard } from 'react-native';
 ...
 export class VerificationCodeView extends ViewState {
     ...
     handlePastedContent = async (pastedContent, index) => {
         ...
         pastedContent.split('').forEach((curChar, currPastedIndex) => {
             if (index + currPastedIndex >= this.viewModel.inputRefArray.length) return;
             this.viewModel.verificationCodeValue[index + currPastedIndex] = curChar;
             this.viewModel.inputRefArray[index + currPastedIndex].focus();
         });

         this.viewModel.isPasting = false;

         if (pastedContent.length + index >= this.viewModel.inputRefArray.length) {
             await this.submitVerificationCode();
         }
     }

     submitVerificationCode = () => this.runLongOperation(async () => {
